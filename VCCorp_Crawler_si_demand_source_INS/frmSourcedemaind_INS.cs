...
namespace VCCorp_Crawler_si_demand_source_INS
{
    public partial class frmSourcedemaind_INS : Form
    {
        ...
        private async void CrawlerAndSend()
        {
            try
            {
                string dtgettime = _listsidemandsource[_indexCurr].update_time_crawl;
                try
                {
                    DateTime dttime = DateTime.Parse(dtgettime);
                    DateTime dtnow = DateTime.Now;
                    var dtday = dtnow - dttime;
                    var dtdatyu = dtday.TotalDays * 24 * 60 * 60;
                    int intdtday = Convert.ToInt32(dtdatyu);
                    int intettime = Convert.ToInt32(_listsidemandsource[_indexCurr].frequency);
